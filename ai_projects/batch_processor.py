...
def process_single_alert(alerts,ti_cache_data,ai_cache_data,timing):
  try:
    ...
    ai_output,token_count,alert_ti_cache_data,alert_ai_cache_data,response_key=day1_alertclassifier.classify_alert(alerts['alert'],alert_ti_cache_data,alert_ai_cache_data,alert_timing)
    print(ai_output)
    if token_count:
...
def test_function(alerts):
    try:
        ...
        for results in as_completed(future):
            ...
            print(f"Alert:{result['alert']['name']}finished in {result['TotalTime']}s | Class: {result['Classification']}")
...
#test_function(test_data.test_cases_1)
